...
def compute_data_docker(n, file):
    ...
    for line in Lines:
        if count < n:
            if line != "\n":
                ...
                line_split = line_string.split("-redis_")
                ...
                # This "name" should be changed according to the name of the container we want to plot.
                if name == 'redis_1':
    ...
    ax.plot(x, y1, 'o-', markersize=2, label='Redis Network Input')
    ax.plot(x, y2, 'o-', markersize=2, label='Redis Network Output')
    ax.plot(x, y3, 'o-', markersize=2, label='Redis Block Input')
    ax.plot(x, y4, 'o-', markersize=2, label='Redis Block Output')
    ...
    plt.title("Redis container data")
    plt.xlim([-5, 95])
    plt.show()
    for i in range(len(data)):
...
if __name__ == '__main__':
    ...
    compute_data_docker(90, "output.txt")
